...
def withdraw_money(person, money):
    if person['money'] - money != 0:                                            #заменил == на !=
        person['money'] -= money
        return 'На счете осталось {} рублей'.format(check_account(person), 2)   #заменил на вывод остатка ради интереса
    else:
...
def start():
    ...
    if person and is_pin_valid(person, pin_code):                                #убрал while
        choice = int(input('Выберите пункт:\n'
                           ...
                           'Ваш выбор:'))
        if choice == 3:
            print('Всего доброго!')                                              #заменил break
